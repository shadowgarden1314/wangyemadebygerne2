...
let bg_img_preinstall = {
    ...
    "3": "https://www.loliapi.com/acg/", // 随机ACG
    "4": "https://t.mwm.moe/ycy", // 随机动漫
    "5": "https://cdn.jsdelivr.net/gh/shadowgarden1314/imagegeren/Lycoris.png", // 随机动漫
	"6": "https://tuapi.eees.cc/api.php?category=dongman&type=302", // 随机动漫
	"7": "https://www.dmoe.cc/random.php", // 随机动漫
	"8": "https://api.yimian.xyz/img", // 随机动漫
	"9": "https://tuapi.eees.cc/api.php?category=fengjing&type=302", // 随机动漫
	"10": "https://tuapi.eees.cc/api.php?category=biying&type=302", // 随机动漫
	"11": "https://tuapi.eees.cc/api.php?category=meinv&type=302", // 随机动漫
	"12": "https://api.shanrenyi.top/img", // 随机动漫
};
...
function setBgImgInit() {
    ...
    switch (bg_img["type"]) {
        case "1":
            $('#bg').attr('src', `./img/background${1 + ~~(Math.random() * 41)}.webp`) //随机默认壁纸
            break;
        case "2":
        ...
        case "4":
            ...
            break;
	    case "5":
            $('#bg').attr('src', bg_img_preinstall[5]); //随机动漫
            break;
	    case "6":
            $('#bg').attr('src', bg_img_preinstall[6]); //随机动漫
            break;
	    case "7":
            $('#bg').attr('src', bg_img_preinstall[7]); //随机动漫
            break;
	    case "8":
            $('#bg').attr('src', bg_img_preinstall[8]); //随机动漫
            break;
	    case "9":
            $('#bg').attr('src', bg_img_preinstall[9]); //随机动漫
            break;
	    case "10":
            $('#bg').attr('src', bg_img_preinstall[10]); //随机动漫
            break;
	    case "11":
            $('#bg').attr('src', bg_img_preinstall[11]); //随机动漫
            break;
	    case "12":
            $('#bg').attr('src', bg_img_preinstall[12]); //随机动漫
            break;		
    }
};
